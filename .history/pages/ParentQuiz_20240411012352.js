...
export default function ParentQuiz() {
  ...
  const handleAnswerSelect = (answer, points) => {
    // Convert points to a number, defaulting to 0 if it's not a valid number
  ...
  const calculateOutcome = () => {
    ...
    if (totalScore >= 0 && totalScore <= 3) {
      message = `You're doing great about your ${headspace}! Keep it up!`;
    } else if (totalScore > 3 && totalScore <= 6) {
      message = `You're doing fantastic about your ${headspace}! Keep up the good work!`;
    } else if (totalScore > -7 && totalScore <= -5) {
      message = `It sounds like you're having a tough time with your ${headspace}. Consider seeking professional help.`;
    } else if (totalScore > -5 && totalScore <= -3) {
      message = `It sounds like you're struggling with your ${headspace}. Try some mental exercises to improve your mood.`;
    } else if (totalScore > -2 && totalScore <= -1) {
      message = `It seems like you're having a bit of difficulty with your ${headspace}. Mental exercises might help.`;
    }
    setOutcome(message + ` Your score: ${totalScore}`);
  };
      ...
      <main className={`${styles.main}`}>
        ...
        <Navbar />
          ...
          <div className={styles.buttonContainer}>
            {step > 1 && !quizComplete && (
              <button className={`${styles.secondaryButtonQuiz}`} onClick={handlePreviousStep} tabindex="0">Back</button>
            )}
            {!quizComplete ? (
              <button className={`${styles.primaryButtonQuiz}`} onClick={step === 4 ? handleSubmitQuiz : handleNextStep} tabindex="0">
                {step === 4 ? "Submit" : "Next"}
              </button>
            ) : (
              <></>
